		...
		public static string CalcEqual ()
		{
			...
			else if (stringAnswer == "")
			{
                ...
                switch (calcOperation)
                {
                    ...
                    case Operator.eInverse:   // 1 / x
                        numericAnswer = 1 / firstNumber;
                        validEquation = true;
                        break;

                    case Operator.eSquareDegree:   // x ^ 2
                    ...
                    case Operator.eFactorial:
                        int factorial = 1;
                        for (int i = 2; i <= firstNumber; i++) // цикл начинаем с 2, т.к. нет смысла начинать с 1
                        {
                    ...
                    case Operator.eCubeRoot:   // кубический корень
                        numericAnswer = Math.Sign(firstNumber) * Math.Pow(Math.Abs(firstNumber), 1 / 3.0);				
				//Math.Pow(firstNumber, 1 / 3);
                        validEquation = true;
                        break;
